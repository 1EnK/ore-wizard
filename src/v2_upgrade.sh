...
for keypair in "$keypair_dir"/*.json; do
    echo "upgrading ORE from V1 to V2 for '$(basename "$keypair")'..."
    
    # Build the command to upgrade the ORE token
    command="ore upgrade --rpc \"$rpc_url\" --keypair \"$keypair\" --priority-fee \"$priority_fee\""
